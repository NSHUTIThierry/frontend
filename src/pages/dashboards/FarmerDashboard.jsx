...
function iconFor(key) {
  var m = {farm:'⊞',crops:'🌾',reports:'📋',agronomist:'👨‍🔬',appointments:'📅',marketplace:'🛒',sell:'💰',alerts:'🔔',knowledge:'📚',payments:'💳',profile:'👤'};
  return m[key] || '📄';
}

function makeItems(prefix) {
  var labels = {farm:'My Farm',crops:'My Crops',reports:'Reports',agronomist:'Find Agronomist',appointments:'Appointments',marketplace:'Marketplace',sell:'Sell Produce',alerts:'Alerts',knowledge:'Knowledge Base',payments:'Payments',profile:'Profile'};
  return Object.keys(labels).map(function(k) { return {key: k, label: labels[k]}; });
}

export default function FarmerDashboard({ user, onLogout }) {
  const first = user?.name?.split(' ')[0] || 'Marie';
  const inits = user?.initials || 'MU';
  var sT = useState('farm');
    ...
    return (
      <div className="fd-body">
        <div className="fd-card">
          ...
          <table className="dtable"><thead><tr><th>Crop</th><th>Area</th><th>Stage</th><th>Health</th></tr></thead>
          <tbody>
...
function placeholderContent(label) {
  return <div className="dcard"><h4>{label}</h4><p style={{fontSize:'11px',color:'#666'}}>This section is coming soon.</p></div>;
}
      ...
      <div className="dash-wrap">
        <div className="dash-sb">
          ...
          <div className="dash-nav">
            {items.map(function(it) {
              return <div key={it.key} className={'di ' + (tab===it.key?'act':'')} onClick={function(){setTab(it.key);}}><span className="dii">{iconFor(it.key)}</span><span>{it.label}</span></div>;
            })}
          </div>
        ...
        <div className="dash-main">
          ...
          <div className="dash-body">
            {tab === 'farm' && <div className="farmer-dash-hero"><h2>Welcome back, <span id="farmer-welcome">{first}</span>! 🌾</h2><p>Manage your farm, connect with experts, and sell your produce all in one place.</p></div>}
            {tab === 'farm' ? doneSection() : placeholderContent(items.find(function(x){return x.key===tab;}).label)}
          </div>
        </div>
      </div>
    </div>
